...
def str_to_time(timestamp,tzsource=None):
    ...
    try:
        ...
        return None
    if not time.tzinfo and tzsource:
        if isinstance(tzsource, str):
            eastern = pytz.timezone(tzsource)
...
def daytime(time,daycycle):
    ...
    time_new, time_zone, date = time.time(), time.tzinfo, time.date()
    year, month = str(date).split("-", 1)
    if year in daycycle and month in daycycle[year]:
        sunset = parse(str(date) + " " + daycycle[year][month]['sunset'])
        sunrise = parse(str(date) + " " + daycycle[year][month]['sunrise'])
        time_zone_data = daycycle['timezone']
        if time_zone == None:
            american = pytz.timezone(time_zone_data)
            time_new = american.localize(time).time()
        else:
            test = pytz.timezone(time_zone_data)
            sunset = test.localize(sunset)
            sunrise = test.localize(sunrise)
